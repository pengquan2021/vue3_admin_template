...
//商品图片 -> 适配 el-upload 的 file-list 类型
let imgList = ref<UploadUserFile[]>([])
// 上传请求头，携带 token，保证后端鉴权通过
...
const initHasSpuData = async (spu: SpuData) => {
  //存储已有的SPU对象,将来在模板中展示
  SpuParams.value = spu
  // Element Plus el-select 比较 value 使用全等(===)，
  // 为避免 string/number 不一致导致只显示 ID，不显示 label，这里统一转成数字
  SpuParams.value.tmId = Number(spu.tmId)
  ...
  //SPU对应商品图片 => 转成 el-upload 需要的 UploadUserFile 结构
...
const initAddSpu = async (c3Id: number | string) => {
  ...
  Object.assign(SpuParams.value, {
    id: undefined, // 清空 id，确保添加时不被识别为修改
    category3Id: '', //收集三级分类的ID
    spuName: '', //SPU的名字
    description: '', //SPU的描述
    tmId: '', //品牌的ID
